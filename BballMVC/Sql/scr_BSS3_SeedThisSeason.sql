...
--	Declare @LoopLimit int = @varLgAvgGamesBack * (Select Count(*) From @TeamTable)
	...
	Declare @SeedGameDate Date = ( SELECT min([StartDate])
														  FROM [00TTI_LeagueScores].[dbo].[SeasonInfo]
														  where LeagueName = @leagueName   AND Season = @Season
														)
	Set @SeedGameDate = DATEADD(d, -1, @SeedGameDate);
	Declare @LgAvgScore int
	If @LeagueName = 'NBA'
		Set @LgAvgScore = 220
	else
	 	Set @LgAvgScore = 160
...
BEGIN -- 3) Delete This Years SEEDS if they exist
 ...
 Delete From BoxScores Where LeagueName  = @LeagueName AND Season = @Season AND Source = @Seeded
 -- return
END -- 3)

BEGIN -- 4) Insert Seeds
	Select '====>', @LeagueName, @Season, @SeedGameDate

	-- Loop 1) Teams	2) Venues 3) @varLgAvgGamesBack times

	Set @Team = ''
	While 1 = 1		-- Loop Teams times
	BEGIN	-- 1) Teams Loop
		...
		While 1 = 1		-- Loop @varLgAvgGamesBack times
		BEGIN	-- 2) @ctrLoop Loop
			Set @ctrLoop = @ctrLoop + 1;
			If @ctrLoop > @varLgAvgGamesBack
				BREAK;
...
Select 0 as Exclude
-- 1/7
, @LeagueName, @SeedGameDate, @RotNum, @Team, '', @Venue, ''
-- 2/5
, @Season, @SubSeason, 0, @MinsPerGame, 0
-- 3/6
, @LgAvgScore, @LgAvgScore, @LgAvgScore, @LgAvgScore, @LgAvgScore, @LgAvgScore

-- 4/8
, @LgAvgScore / 4, @LgAvgScore / 4, @LgAvgScore / 4, @LgAvgScore / 4, @LgAvgScore / 4, @LgAvgScore / 4, @LgAvgScore / 4, @LgAvgScore / 4
-- 5/3 Actual Made us
